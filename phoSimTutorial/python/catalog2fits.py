...
FORMAT_MAP = {'A':np.str, 'L':np.bool_, 'B':np.int8, 'I':np.int16, 'J':np.int32, 
              'K':np.int64, 'E':np.float32, 'D':np.float64}

def catalog2Fits(args):
    """Reformat an ASCII catalog to FITS BINTABLE, using a column definition file. 
    """

    descr_text = 'Transform an ASCII Catalog to FITS BINTABLE format'
    ...
    # Fetch column definitions, which must include the four key column attributes.
    rowList = []
    ...
    # Populate the catalog object, skipping comment lines (beginning with '#')
    cat = Catalog(nameList, formList)
    with open(args.inFile, 'r') as inFile:
        for line in csv.reader(inFile, delimiter=',', quotechar='"'):
